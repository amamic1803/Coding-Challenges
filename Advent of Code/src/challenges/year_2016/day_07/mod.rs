        ...
        .filter(|line| {
            let is_tls = |elem: &str| {
                for i in 0..(elem.chars().count() - 3) {
                    if elem.chars().nth(i).unwrap() == elem.chars().nth(i + 3).unwrap() && elem.chars().nth(i + 1).unwrap() == elem.chars().nth(i + 2).unwrap() && elem.chars().nth(i).unwrap() != elem.chars().nth(i + 1).unwrap() {
                        return true;
                    }
                }
                false
...
fn part2(input: &str) -> String {
    let split_re = Regex::new(r"\[\w*]").unwrap();
        ...
        .filter(|line| {
            ...
            let hypernet = split_re.find_iter(line).map(|mat| &line[(mat.start() + 1)..(mat.end() - 1)]).filter(|elem| elem.chars().count() >= 3).collect::<Vec<&str>>();
