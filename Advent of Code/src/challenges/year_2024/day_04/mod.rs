...
fn part1(input: &str) -> String {
    ...
    for (i, row) in text.iter().enumerate() {
        for (j, &c) in row.iter().enumerate() {
            ...
            // NE
            if top_padding && right_padding && check_xmas(|k| text[i - k][j + k] == xmas_word[k]) {
                xmas_count += 1;
            }
            ...
            // SE
            if bottom_padding && right_padding && check_xmas(|k| text[i + k][j + k] == xmas_word[k]) {
                xmas_count += 1;
            }

            // SW
            if bottom_padding && left_padding && check_xmas(|k| text[i + k][j - k] == xmas_word[k]) {
                xmas_count += 1;
            }
        }
    }
...
fn part2(input: &str) -> String {
    ...
    for (i, row) in text.iter().enumerate() {
        for (j, &c) in row.iter().enumerate() {
            ...
            // SE
            if bottom_padding && right_padding && check_x_mas(|m, n| text[i + m][j + n] == mas_word[m][n]) {
                mas_count += 1;
            }

            // SW
            if bottom_padding && left_padding && check_x_mas(|m, n| text[i + n][j - m] == mas_word[m][n]) {
                mas_count += 1;
            }

            // NW
            if top_padding && left_padding && check_x_mas(|m, n| text[i - m][j - n] == mas_word[m][n]) {
                mas_count += 1;
            }

            // NE
            if top_padding && right_padding && check_x_mas(|m, n| text[i - n][j + m] == mas_word[m][n]) {
                mas_count += 1;
            }
        }
    }
