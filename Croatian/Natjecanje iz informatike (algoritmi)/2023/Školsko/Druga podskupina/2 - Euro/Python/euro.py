...
sve_moguce_drzave.sort()  # sortiramo drzave jer ce kasnije biti potrebne sortirane kako bi bile ispisane abecednim redom (uvjet 1.)
    ...
    while len(proizvodi_u_drzavi) != 0:
        ...
        for x in range(len(proizvodi_u_drzavi)):
            if proizvodi_u_drzavi[x - izbrisano][0] == najmanja_cijena:
                ...
                izbrisano += 1
        oni_s_najmanjom_cijenom = set(oni_s_najmanjom_cijenom)  # pretvori u set i nazad da se izbrisu duplikati (do kojih bi doslo kada bi isti upis bio ponovljen vise od jednom, primjerice dva proizvoda bi imala isto ime, zemlju, cijenu)
        oni_s_najmanjom_cijenom = list(oni_s_najmanjom_cijenom)
        oni_s_najmanjom_cijenom.sort()  # sortiramo abecedno te proizvode u drzavi koji imaju najmanju cijenu jer ih tim redoslijedom treba ispisati

        for pr in oni_s_najmanjom_cijenom:
            print(f"{pr}-{drzava}-{f'{round(najmanja_cijena, 2): .2f}'.replace('.', ',')}")  # ispis (output)
